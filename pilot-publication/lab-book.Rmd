...
combined.outcome.results <- lapply(ci.levels, function(ci.level) {
    lapply(cell.references, function(cell.reference) {
        ...
        ci <- ci.data[ci.data[, 1] == ci.level, c("lower", "upper")]
        combined <- NA
        if (!all(is.na(ci))) {
            combined <- sprintf("%0.2f", c(point.estimate, ci))
...
create_ci_level_tables <- function(table.name, ci.data) {
    ...
    ci.table <- matrix(table.data, ncol = length(column.names)) %>%
        ...
        tibble::add_column(row.names, .before = 1, .name_repair = "minimal") %>%
...
outcome.results.data <- enframe(outcome.results$point.estimates)
table.names <- strsplit(names(outcome.results$point.estimates), split = "\\|\\|\\.")
```
