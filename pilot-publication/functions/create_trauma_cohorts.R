create_trauma_cohorts <- function(data) {
  ...
  get_severe_injuries <- function(aiscodes) {
    ...
    severe.codes <- grep("\\.[3-6]$", aiscodes, value = TRUE)
    return(severe.codes)
  }
  ...
  count_unique_regions <- function(ais_codes) {
    ...
    num.regions <- length(unique(regions))
    return(num.regions)
  }
  ...
  count_severe_regions <- function(dataset) {
    ...
    severe.injuries <- get_severe_injuries(codes)
    # Count unique regions using count_unique_regions()
    num.regions <- count_unique_regions(severe.injuries)
    return(num.regions)
  }
