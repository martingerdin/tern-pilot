...
We measured a large number of outcomes to help plan and assess the feasibility of a full scale trial. A list of outcomes is available in Supplementary Table S1. Our main feasibility outcomes were: 
...
We used the `r version$version.string` Statistical Software for all analyses [@R_2024]. We analysed all data, including the feasibility outcomes, using descriptive statistics and did not perform any formal hypothesis tests [@Lee2014]. Quantitative variables are summarised as median and interquartile range. Qualitative variables are presented as absolute numbers and percentages. Additional analyses performed according to the original protocol are described in the Supplementary Materials and their results shown in Supplementary Tables S1-12..
...
The percentage of patients who consented to follow up was `r p.consent`% and the percentage of residents who consented to training was `r recruitment.rate.residents`%. Among patients, the lost to follow up rate was `r rate.lost.to.follow.up`%. Among residents, the pass rate was `r pass.rate`%. The missing data rate ranged from `r min.missing.data` to `r max.missing.data`%, with details for selected variables shown in Table \@ref(tab:sample-characteristics) and in Supplementary Table S1. The variables with the maximum amount of missing data were in the cost of treatment, reported in Supplementary Tables S1-12. 
